  ...
  where
    ...
    go !pos slots
      ...
      where
        ...
        remain = size - pos
        init
          | remain >= word_sz = error "remain" -- -1
          | otherwise         = error "otherwis" -- (1 `shiftL` remain) - 1
...
-- toStgWord = error "toStgWord"
-- fromStgWord = error "fromStgWord"
