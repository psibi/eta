...
module ETA.Interactive.ByteCodeGen ( UnlinkedBCO, byteCodeGen, coreExprToBCOs ) where
...
import ETA.Interactive.BitMap
import ETA.Interactive.Types

byteCodeGen = error "byteCodeGen"
-- coreExprToBCOs = error "coreExprToBCOs"
...
-- wORD_SIZE = error "wORD_SIZE"
dOUBLE_SIZE = error "dOUBLE_SIZE"
wORD64_SIZE = error "word64_size"
   ...
   = ProtoBCO {
     ...
     where
        ...
        maybe_with_stack_check
           | is_ret && stack_usage < fromIntegral (error "aP_STACK_SPLIM dflags") = peep_d
...
arrPtrsHdrSize = error "arrPtrsHdrSize"
smallArrPtrsHdrSize = error "smallArrPtrsHdrSize"
arrWordsHdrSize = error "arrWordsHdrSize"
